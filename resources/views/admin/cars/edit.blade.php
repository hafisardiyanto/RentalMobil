    ...
    <form action="{{ route('admin.cars.update', $car->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-grid-2">
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $car->name) }}" required placeholder="Cth: Avanza Veloz" class="form-input">
                @error('name') <span class="error-msg">{{ $message }}</span> @enderror
            </div>
            <div>
                <label class="form-label">Merek (Brand)</label>
                <input type="text" name="brand" value="{{ old('brand', $car->brand) }}" required placeholder="Cth: Toyota" class="form-input">
        ...
        <div class="form-grid-4">
            <div>
                ...
                <input type="text" name="license_plate" value="{{ old('license_plate', $car->license_plate) }}" required placeholder="B 1234 CD" class="form-input">
                @error('license_plate') <span class="error-msg">{{ $message }}</span> @enderror
            </div>
            <div>
                <label class="form-label">Tahun</label>
                <input type="number" name="year" value="{{ old('year', $car->year) }}" required min="2000" placeholder="Cth: 2022" class="form-input">
                @error('year') <span class="error-msg">{{ $message }}</span> @enderror
            </div>
            <div>
                <label class="form-label">Harga (Rp)</label>
                <input type="number" name="price_per_day" value="{{ old('price_per_day', $car->price_per_day) }}" required placeholder="300000" class="form-input">
        ...
        <div class="upload-section">
            ...
            <div class="preview-container" style="display: flex; margin-bottom: 1.5rem;">
            ...
            <div class="upload-box">
                <input type="file" name="images[]" id="imageInput" multiple accept="image/jpeg,image/png,image/jpg" class="upload-input">
